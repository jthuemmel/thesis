...
class Experiment(DistributedTrainer):
    ...
    def create_loss(self):
        def loss_fn(ens: torch.Tensor, obs: torch.Tensor, visible: torch.BoolTensor):
            score = f_kernel_crps(obs, ens)
            mask = torch.logical_and(self.land_sea_mask, ~visible)
            rate_correction = einops.reduce(mask, 'b n d -> b 1 1', 'sum').clamp(min=1) / self.world.num_elements
