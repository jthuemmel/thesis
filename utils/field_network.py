...
class StochasticWeatherField(Module):
    def __init__(self, cfg: dataclass):
        ...
        self.decoder = Interface(cfg.dim, cfg.num_compute_blocks, dim_heads= cfg.dim_heads, dim_ctx=cfg.dim_noise)
    ...
    def step(self, 
        ...
        # Interface network computation over src
        for block in self.network:
            src, z = block(src, z, noise)

        # Interface network over src and query
        x = torch.cat([src, q], dim = 1)
        x, z = self.decoder(x, z, noise)
