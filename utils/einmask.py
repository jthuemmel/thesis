...
class EinMask(torch.nn.Module):
    def __init__(self, network: NetworkConfig, world: WorldConfig):
        ...
        self.mask_token = torch.nn.Parameter(torch.nn.init.trunc_normal_(torch.zeros(default(network.dim_out, network.dim)), std = default(network.dim_out, network.dim) ** -0.5))
        self.src_positions = torch.nn.Parameter(init_sincos_positions(network.dim, world= world))
        self.tgt_positions = torch.nn.Parameter(init_sincos_positions(default(network.dim_out, network.dim), world= world))
        ...
        self.to_tokens = torch.nn.Sequential( # maybe keep vv out of c here for delayed mixing
            EinMix(f'b {world.field_pattern} -> b ({world.token_pattern}) c',
                ...
                c = default(network.dim_in, network.dim), **world.token_sizes, **world.patch_sizes),
            EinMix(f'b ({world.token_pattern}) c -> b ({world.token_pattern}) d',
                weight_shape = f'v d c',
                d = network.dim, c = default(network.dim_in, network.dim), **world.token_sizes),
            torch.nn.RMSNorm(network.dim)
        )
        
        self.to_decoder = torch.nn.Sequential(
            torch.nn.RMSNorm(network.dim),
            torch.nn.Linear(network.dim, default(network.dim_out, network.dim), bias = False),
        )
        ...
        self.to_output = torch.nn.Sequential(
            EinMix(f'b ({world.token_pattern}) d -> (k b) {world.field_pattern}',
                   ...
                   d = default(network.dim_out, network.dim), k = network.num_tails, **world.patch_sizes, **world.token_sizes),
        ...
        self.decoder = torch.nn.ModuleList([
                TransformerBlock(dim= default(network.dim_out, network.dim), num_heads= network.num_decoder_heads) 
